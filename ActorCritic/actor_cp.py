...
def getInitTheta(M):
    return np.random.randn((M+1)**4, len(ACTIONS))
...
def runEpisode(theta, w, alpha_theta, alpha_w, gamma, M, use_cos=True):
    ...
    while not isTerminalState(s):
        ...
        # phi_s_next = getFeatureVec(s_next, M) #not needed as feature creation taken care while calculating value
...
def runOAC_Ntimes(N, nIters, M, gamma , theta, w, alpha_theta, alpha_w, Jeps, alpha_theta_red_factor, alpha_w_red_factor, initW, hpara=""):
    ...
    plt.savefig(f"./tuning/CP/{hpara}/SC_AE_{hpara}_({N}_{nIters}_{Jeps}_{M}_{gamma}_{alpha_theta}_{alpha_w}).jpg")
    plt.close()
    ...
    plt.savefig(f"./tuning/CP/{hpara}/SC_MStd_{hpara}_({N}_{nIters}_{Jeps}_{M}_{gamma}_{alpha_theta}_{alpha_w}).jpg")
    plt.show(block=False)
    plt.close()
    ...
    plt.savefig(f"./tuning/CP/{hpara}/SC_Std_{hpara}_({N}_{nIters}_{Jeps}_{M}_{gamma}_{alpha_theta}_{alpha_w}).jpg")
    plt.close()
...
def tuningParameters(N, nIters, M, gamma, alpha_w, alpha_theta, use_cos, Jeps, alpha_theta_red_factor, alpha_w_red_factor, initW):
    ...
    feat_arr = [0.0001, 0.0005, 0.001,  0.005, 0.009, 0.01, 0.05, 0.09, 0.1, 0.2, 0.5, 0.9]
    ...
    # feat_arr = [0.0001, 0.0005, 0.001, 0.005, 0.01] 
    # # feat_arr = [ 0.1, 0.2, 0.5]
...
def main():
    ...
    runOAC_Ntimes(N, nIters, M, gamma , theta, w, alpha_theta, alpha_w, Jeps, alpha_theta_red_factor, alpha_w_red_factor, initW, hpara="final_run")
